...
BEGIN
    ...
    -- [문제] dept_lists에 담긴 데이터를 모두 출력하라. (오영국)
    -- 10,20,30,40,55,56,57
    FOR cnt IN (SELECT * FROM dept) LOOP
      DBMS_OUTPUT.PUT_LINE('[' || cnt.deptno || '] ' || 
         dept_lists(cnt.deptno).deptno || ', ' || 
         dept_lists(cnt.deptno).dname || ', ' || 
         dept_lists(cnt.deptno).loc);
    END LOOP;
END;
/
