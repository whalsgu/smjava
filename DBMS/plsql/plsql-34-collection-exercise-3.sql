...
BEGIN
    ...
    -- [문제] dept_lists에 담긴 데이터를 모두 출력하라. (권철주)
    FOR deptx IN (SELECT deptno FROM DEPT) LOOP
        DBMS_OUTPUT.PUT_LINE('* ' || deptx.deptno);
        DBMS_OUTPUT.PUT_LINE(dept_lists(deptx.deptno).deptno || ', '  ||
                             dept_lists(deptx.deptno).dname || ', ' || 
                             dept_lists(deptx.deptno).loc);
    END LOOP;
END;
/
